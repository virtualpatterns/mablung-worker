...
(async () => {
  ...
  workerServerClass = await Promise.resolve(`${_commander.default.workerServerClassPath}`).then(s => _interopRequireWildcard(require(s))); // URL.pathToFileURL(Command.workerServerClassPath))
  ...
  await new workerServerClass().import(_commander.default.importPath);
})();
